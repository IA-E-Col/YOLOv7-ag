...
def feature_visualization(x, module_type, stage, n=32, save_dir="runs/detect/exp"):
    ...
    if 'Detect' not in module_type:
        import random
        rnd = random.randint(1, 900)
        save_dir = "/pools/apollon/ummisco/data/db/ecolplus/AM_Output"
        batch, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = save_dir+"/"+f"stage{stage}_{module_type.split('.')[-1]}_features_{rnd}.png"  # filename
